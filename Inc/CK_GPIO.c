...
void CK_GPIOx_Init(GPIO_TypeDef* GPIOx, uint16_t GPIOx_Pin, CK_GPIOx_Mode GPIOx_Mode, CK_GPIOx_AFx GPIOx_AF,
...
void CK_GPIOx_ClockEnable(GPIO_TypeDef* GPIOx){
	uint16_t port_clk = ((uint32_t)GPIOx - (GPIOA_BASE)) / ((GPIOB_BASE) - (GPIOA_BASE));
	RCC->AHB1ENR |= 1u<<port_clk;
}

void CK_GPIOx_SetPin(GPIO_TypeDef* GPIOx, uint16_t GPIOx_Pin){

	GPIOx->BSRR |= 1u<<GPIOx_Pin;

}

void CK_GPIOx_ClearPin(GPIO_TypeDef* GPIOx, uint16_t GPIOx_Pin){

	GPIOx->BSRR |= 1u<<(GPIOx_Pin+16);

}

void CK_GPIOx_TogglePin(GPIO_TypeDef* GPIOx, uint16_t GPIOx_Pin){

	if((GPIOx->ODR & 1u << GPIOx_Pin) != 0x00){
...
uint8_t CK_GPIOx_ReadPin(GPIO_TypeDef* GPIOx ,uint16_t GPIOx_Pin){
